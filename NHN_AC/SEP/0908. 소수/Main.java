public class Main {
    ...
    private static int isPrimeSolution(int start, int end) {
        int count = 0;
        boolean notPrimeNumbers[] = isPrime(end);
    ...
    private static boolean[] isPrime(int num) {
        boolean notPrimeNumbers[] = new boolean[num + 1];
        notPrimeNumbers[1] = true;

        for (int i = 2; i < Math.pow(num, 0.5) + 1; i++) {
            int j = 2;
            while (j * i <= num) {
                if (!notPrimeNumbers[i * j])
    ...
    public static void main(String[] args) {
        for (int i = 0; i < TEST_CASES.length; i++) {
            System.out.println("Test Case " + (i + 1) + " = "
                    + test(TEST_CASES[i][0], TEST_CASES[i][1], TEST_CASES_RESULTS[i]));
        }
    ...
    private static final int[][] TEST_CASES = {{1, 10}, {1, 50}, {1, 1000}, {1, 100000}, {5, 20},
            {11, 123}, {55, 5555}, {1000, 100000}};

    private static final int[] TEST_CASES_RESULTS = {4, 15, 168, 9592, 6, 26, 716, 9424};
